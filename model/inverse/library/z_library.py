...
# 从现有模块导入所需组件
try:
    from ...forward.g2s.g2s_model import NMR_VAE, LocalNMRDataset, load_raw_data
    from ...shared.coarse_graph import NUM_SU_TYPES, SU_DEFS, SU_MAX_DEGREE, SU_PPM_RANGES
except ImportError:
    from model.forward.g2s.g2s_model import NMR_VAE, LocalNMRDataset, load_raw_data
    from model.shared.coarse_graph import NUM_SU_TYPES, SU_DEFS, SU_MAX_DEGREE, SU_PPM_RANGES
...
try:
    from ...shared.inverse_common import (
        ...
        SU_EXTERNAL_CONNECTIONS, SU_FIXED_CONNECTIONS, UNSATURATED_PAIRS,
        FORBIDDEN_CONNECTIONS, validate_connection,
        check_external_connection_requirement, get_port_pattern_degrees
    )
except ImportError:
...
try:
    from ..hop1_adjuster import can_match_ports_exact
except ImportError:
    from model.inverse.hop1_adjuster import can_match_ports_exact
...
def _can_match_ports(neighbors: List[int], port_sets: List[Set[int]]) -> bool:
    """检查邻居列表是否能匹配端口集合"""
    return bool(can_match_ports_exact(neighbors, port_sets))
...
def validate_hop1_multiset(center_su: int, hop1_multiset: tuple) -> Tuple[bool, List[str]]:
    ...
    if center_su in HOP1_PORT_COMBINATIONS:
        ...
        allowed_degrees = get_port_pattern_degrees(port_sets)
        if allowed_degrees and int(total_degree) not in set(int(x) for x in allowed_degrees):
            violations.append(f"Degree {total_degree} not in allowed {list(allowed_degrees)}")
        else:
...
def _classify_special_anchor_mode(center_su: int, hop1_multiset: tuple) -> Optional[str]:
    degree_i = int(len(tuple(hop1_multiset)))
    if int(degree_i) not in {2, 3}:
        return None
    if int(center_su) == 19:
        fixed_count = int(sum(1 for x in tuple(hop1_multiset) if int(x) in {2, 28, 29}))
    elif int(center_su) == 20:
        fixed_count = int(sum(1 for x in tuple(hop1_multiset) if int(x) in {0, 27}))
    else:
        return None
...
def _fixed_anchor_multiset(center_su: int, hop1_multiset: tuple) -> tuple:
    if int(center_su) == 19:
        fixed_types = {2, 28, 29}
    elif int(center_su) == 20:
        fixed_types = {0, 27}
        ...
        fixed_types = set()
    return tuple(sorted(int(x) for x in tuple(hop1_multiset) if int(x) in fixed_types))


def _free_neighbor_multiset(center_su: int, hop1_multiset: tuple) -> tuple:
    if int(center_su) == 19:
        fixed_types = {2, 28, 29}
    elif int(center_su) == 20:
        fixed_types = {0, 27}
        ...
        fixed_types = set()
    return tuple(sorted(int(x) for x in tuple(hop1_multiset) if int(x) not in fixed_types))
...
def filter_subgraph_library(
    ...
    # 8.5. 按 (center_su, target_degree) 统计特殊SU内部连接度模式的核心NMR范围
    special_mode_agg: Dict[Tuple[int, int], List[Dict[str, float]]] = defaultdict(list)
    special_su_types = set(int(x) for x in SPECIAL_DEGREE_PRIORS.keys())
    for tpl_key, tpl_info in filtered_templates.items():
        center_su_idx, hop1_multiset, _ = tpl_key
        center_su_idx = int(center_su_idx)
        if center_su_idx not in special_su_types:
            continue
        degree_i = int(len(tuple(hop1_multiset)))
        if degree_i not in set(int(k) for k in SPECIAL_DEGREE_PRIORS.get(center_su_idx, {}).keys()):
            continue
        mu = tpl_info.get('center_mu', 0.0)
        if mu is None or float(mu) == 0.0:
            continue
        special_mode_agg[(center_su_idx, degree_i)].append({
            'mu': float(mu),
            'mu_min': float(tpl_info.get('mu_min', mu)),
            'mu_max': float(tpl_info.get('mu_max', mu)),
            'sample_count': int(tpl_info.get('sample_count', 1)),
    ...
    special_mode_rows = []
    for (su_idx, degree_i), items in special_mode_agg.items():
        mu_values = [it['mu'] for it in items]
        weights = [it['sample_count'] for it in items]
        all_mu_min = [it['mu_min'] for it in items]
        all_mu_max = [it['mu_max'] for it in items]
        stats_mode = _compute_common_mu_stats(mu_values, weights)
        if stats_mode is None:
            continue
        special_mode_rows.append({
            'center_su_idx': int(su_idx),
            'center_su': su_names[su_idx],
            'target_degree': int(degree_i),
            'mu_median': float(stats_mode['mu_median']),
            'mu_common_min': float(stats_mode['mu_common_min']),
            'mu_common_max': float(stats_mode['mu_common_max']),
            'mu_q05': float(stats_mode['mu_q05']),
            'mu_q95': float(stats_mode['mu_q95']),
            'mu_global_min': float(min(all_mu_min)),
            'mu_global_max': float(max(all_mu_max)),
            'n_templates': len(items),
            'sample_count_total': sum(weights),
    ...
    if special_mode_rows:
        special_mode_rows_sorted = sorted(
            special_mode_rows,
            key=lambda r: (r['center_su_idx'], r['target_degree']),
        )
        special_mode_df = pd.DataFrame(special_mode_rows_sorted)
        special_mode_csv_path = os.path.join(out_dir, 'su_special_degree_nmr_common_range_filtered.csv')
        special_mode_df.to_csv(special_mode_csv_path, index=False)
        print(f"  -> 筛选后特殊SU按连接度核心NMR范围已保存: {special_mode_csv_path}")

    # 8.6. 按单/双固定外接模式统计特殊SU范围
    anchor_mode_agg: Dict[Tuple[int, str], List[Dict[str, float]]] = defaultdict(list)
    anchor_mode_detail_rows = []
    for tpl_key, tpl_info in filtered_templates.items():
        center_su_idx, hop1_multiset, _ = tpl_key
        center_su_idx = int(center_su_idx)
        if center_su_idx not in {19, 20}:
            continue
        anchor_mode = _classify_special_anchor_mode(center_su_idx, hop1_multiset)
        if not anchor_mode:
            continue
        mu = tpl_info.get('center_mu', 0.0)
        if mu is None or float(mu) == 0.0:
            continue
        fixed_ms = _fixed_anchor_multiset(center_su_idx, hop1_multiset)
        free_ms = _free_neighbor_multiset(center_su_idx, hop1_multiset)
        anchor_mode_agg[(center_su_idx, str(anchor_mode))].append({
            'mu': float(mu),
            'mu_min': float(tpl_info.get('mu_min', mu)),
            'mu_max': float(tpl_info.get('mu_max', mu)),
            'sample_count': int(tpl_info.get('sample_count', 1)),
        })
        anchor_mode_detail_rows.append({
            'center_su_idx': int(center_su_idx),
            'center_su': su_names[center_su_idx],
            'target_degree': int(len(tuple(hop1_multiset))),
            'anchor_mode': str(anchor_mode),
            'fixed_anchor_count': int(len(tuple(fixed_ms))),
            'fixed_anchor_multiset': f"[{','.join(str(i) for i in fixed_ms)}]",
            'free_neighbor_multiset': f"[{','.join(str(i) for i in free_ms)}]",
            'hop1_multiset': f"[{','.join(str(i) for i in hop1_multiset)}]",
            'mu_median': float(mu),
            'mu_common_min': float(tpl_info.get('mu_min', mu)),
            'mu_common_max': float(tpl_info.get('mu_max', mu)),
            'sample_count': int(tpl_info.get('sample_count', 1)),
        })

    anchor_mode_rows = []
    for (su_idx, anchor_mode), items in anchor_mode_agg.items():
        mu_values = [it['mu'] for it in items]
        weights = [it['sample_count'] for it in items]
        all_mu_min = [it['mu_min'] for it in items]
        all_mu_max = [it['mu_max'] for it in items]
    ...
    if anchor_mode_rows:
        anchor_mode_rows_sorted = sorted(
            anchor_mode_rows,
            key=lambda r: (r['center_su_idx'], r['anchor_mode']),
        )
        anchor_mode_df = pd.DataFrame(anchor_mode_rows_sorted)
        anchor_mode_csv_path = os.path.join(out_dir, 'su_special_anchor_mode_nmr_range_filtered.csv')
        anchor_mode_df.to_csv(anchor_mode_csv_path, index=False)
        print(f"  -> 筛选后特殊SU按单/双固定外接模式范围已保存: {anchor_mode_csv_path}")

    if anchor_mode_detail_rows:
        anchor_mode_detail_rows_sorted = sorted(
            anchor_mode_detail_rows,
            key=lambda r: (r['center_su_idx'], r['anchor_mode'], r['hop1_multiset']),
        )
        anchor_mode_detail_df = pd.DataFrame(anchor_mode_detail_rows_sorted)
        anchor_mode_detail_csv_path = os.path.join(out_dir, 'su_special_anchor_mode_detail_filtered.csv')
        anchor_mode_detail_df.to_csv(anchor_mode_detail_csv_path, index=False)
        print(f"  -> 筛选后特殊SU单/双固定外接明细已保存: {anchor_mode_detail_csv_path}")
