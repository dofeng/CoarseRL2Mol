...
from .RL_state import HexGrid, AromaticCluster, SU_NAMES, count_graph_su_distribution, compute_su_delta
...
def save_cluster(cluster: AromaticCluster, output_dir: str, prefix: str = "cluster") -> str:
    """Save single aromatic cluster to file"""
    ...
    # Center the cluster
    if cluster.sites:
        cx = sum(s.pos2d[0] for s in cluster.sites) / len(cluster.sites)
        cy = sum(s.pos2d[1] for s in cluster.sites) / len(cluster.sites)
        draw_cluster(ax, cluster, offset=(cx, cy))
        
        # Compute bounds
        all_x = [s.pos2d[0] - cx for s in cluster.sites]
        all_y = [s.pos2d[1] - cy for s in cluster.sites]
        margin = 1.0
        ax.set_xlim(min(all_x) - margin, max(all_x) + margin)
        ax.set_ylim(min(all_y) - margin, max(all_y) + margin)
    
    # Count SU types
    n12 = sum(1 for s in cluster.sites if s.su_type == 12)
    n13 = sum(1 for s in cluster.sites if s.su_type == 13)
    
    ax.set_aspect('equal')
    ax.set_title(f"{_pretty_kind(cluster.kind)} (ID={cluster.id}, {cluster.rings} rings, "
                 f"{len(cluster.sites)} sites, 12:{n12}, 13:{n13})", fontsize=11)
    ax.axis('off')
    
    filepath = os.path.join(output_dir, f"{prefix}_{cluster.id}_{cluster.kind}.png")
    plt.savefig(filepath, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close(fig)
    return filepath


def save_all_clusters(clusters: List[AromaticCluster], output_dir: str, 
                      prefix: str = "cluster") -> List[str]:
    """Save all aromatic clusters to individual files"""
    os.makedirs(output_dir, exist_ok=True)
    saved_files = []
    
    for cluster in clusters:
        filepath = save_cluster(cluster, output_dir, prefix)
        saved_files.append(filepath)
    
    return saved_files


def save_overview(clusters: List[AromaticCluster], output_path: str, 
                  title: str = "Aromatic Clusters Overview"):
    """Save overview of all clusters in one image"""
    if not clusters:
        return
    ...
    # Deep copy to avoid modifying original clusters
    clusters = [copy.deepcopy(c) for c in clusters]
    
    # Auto layout
    x_offset = 0
    for cluster in clusters:
        if not cluster.placed and cluster.sites:
            min_q = min(s.axial[0] for s in cluster.sites)
            cluster.translate(int(x_offset - min_q), 0)
            x_offset = max(s.axial[0] for s in cluster.sites) + 3
    
    # Draw all clusters
    for cluster in clusters:
        draw_cluster(ax, cluster)
        cx, cy = cluster.centroid()
        ax.annotate(f"{_pretty_kind(cluster.kind)}\n(ID={cluster.id})", (cx, cy - 1.2),
                    ha='center', fontsize=8, 
                    bbox=dict(boxstyle='round,pad=0.2', fc='white', alpha=0.8))
    
    # Adjust view
    all_x = [s.pos2d[0] for c in clusters for s in c.sites]
    all_y = [s.pos2d[1] for c in clusters for s in c.sites]
    if all_x and all_y:
    ...
    # Legend
    from matplotlib.patches import Patch
    legend = [Patch(fc=_node_color(12), ec='k', label='12: Bridgehead_C'),
              Patch(fc=_node_color(13), ec='k', label='13: Aro_CH')]
...
def save_rigid_result(graph, output_dir: str, title: str = "Rigid Connection Result"):
    """
    Save rigid connection stage result visualization.
    """
    os.makedirs(output_dir, exist_ok=True)
    
    clusters = graph.clusters
    rigid_edges = graph.rigid
    ...
    # Draw all placed clusters
    for cluster in clusters:
        if cluster.placed:
            draw_cluster(ax, cluster)
    
    # Draw rigid edges (thicker red lines)
    ...
    if placed_clusters:
        all_x = [s.pos2d[0] for c in placed_clusters for s in c.sites]
        all_y = [s.pos2d[1] for c in placed_clusters for s in c.sites]
        if all_x and all_y:
            margin = 2.0
            ax.set_xlim(min(all_x) - margin, max(all_x) + margin)
            ax.set_ylim(min(all_y) - margin, max(all_y) + margin)
    ...
    legend_elements = [
        Patch(fc=_node_color(12), ec='k', label='12: Bridgehead_C'),
        Patch(fc=_node_color(13), ec='k', label='13: Aro_CH'),
        Patch(fc=_node_color(10), ec='k', label='10: Aryl_Aro_C (connection)'),
        Line2D([0], [0], color=RIGID_EDGE_COLOR, linewidth=RIGID_EDGE_LW, label='Rigid Edge'),
    ]
    ax.legend(handles=legend_elements, loc='upper right', fontsize=9)
    
    # Add stats
    n_placed = sum(1 for c in clusters if c.placed)
    n_total = len(clusters)
    n_edges = len(rigid_edges)
    stats_text = f"Placed: {n_placed}/{n_total} clusters, Rigid edges: {n_edges}"
    ax.text(0.02, 0.02, stats_text, transform=ax.transAxes, fontsize=10,
            bbox=dict(boxstyle='round', fc='white', alpha=0.9))
    
    output_path = os.path.join(output_dir, "rigid_result.png")
    plt.savefig(output_path, dpi=150, bbox_inches='tight', facecolor='white')
    plt.close(fig)
    
    # Also save individual cluster views
    for cluster in placed_clusters:
        save_cluster(cluster, output_dir, prefix="rigid_cluster")
    
    return output_path


def save_rigid_cluster_distribution(graph, rigid_clusters: List[dict], output_dir: str):
    """
    Save visualization showing individual rigid cluster distributions.
    """
    os.makedirs(output_dir, exist_ok=True)
    
    connected = [rc for rc in rigid_clusters if rc['type'] == 'connected']
    individual = [rc for rc in rigid_clusters if rc['type'] == 'individual']
    
    # 1. Save connected rigid clusters overview
    if connected:
        n_connected = len(connected)
        n_cols = min(4, n_connected)
        n_rows = (n_connected + n_cols - 1) // n_cols
        
        fig, axes = plt.subplots(n_rows, n_cols, figsize=(5 * n_cols, 4 * n_rows), squeeze=False)
        
        for idx, rc in enumerate(connected):
            row, col = idx // n_cols, idx % n_cols
            ax = axes[row, col]
            
            # Get member clusters
            member_ids = rc['member_ids']
            members = [c for c in graph.clusters if c.id in member_ids]
            
            # Draw each member cluster
            for cluster in members:
                if cluster.placed:
                    draw_cluster(ax, cluster)
            
            # Draw rigid edges within this component
            for edge in graph.rigid:
                if edge.cluster_a in member_ids and edge.cluster_b in member_ids:
                    if hasattr(edge, 'line') and edge.line and len(edge.line) >= 2:
                        x1, y1 = edge.line[0]
                        x2, y2 = edge.line[1]
                        ax.plot([x1, x2], [y1, y2], color=(0.9, 0.1, 0.1), 
                                linewidth=2, zorder=2, alpha=0.8)
            
            # Adjust view
            if members:
                all_x = [s.pos2d[0] for c in members for s in c.sites]
                all_y = [s.pos2d[1] for c in members for s in c.sites]
                if all_x and all_y:
                    margin = 1.0
                    ax.set_xlim(min(all_x) - margin, max(all_x) + margin)
                    ax.set_ylim(min(all_y) - margin, max(all_y) + margin)
            ...
            kinds_str = ', '.join(sorted(set(rc['kinds'])))
            ax.set_title(f"Rigid Cluster {idx+1}: {rc['num_clusters']} clusters, {rc['total_rings']} rings\n({kinds_str})",
                        fontsize=9)
        
        # Hide unused subplots
        for idx in range(n_connected, n_rows * n_cols):
            row, col = idx // n_cols, idx % n_cols
            axes[row, col].axis('off')
        
        plt.suptitle(f"Connected Rigid Clusters ({n_connected} total)", fontsize=14, fontweight='bold')
        plt.tight_layout()
        
        path = os.path.join(output_dir, "connected_rigid_clusters.png")
        plt.savefig(path, dpi=150, bbox_inches='tight', facecolor='white')
        plt.close(fig)
    
    # 2. Save individual aromatic clusters overview (compact)
    if individual:
        n_individual = len(individual)
        n_cols = min(6, n_individual)
        n_rows = min(4, (n_individual + n_cols - 1) // n_cols)  # Limit rows
        
        fig, axes = plt.subplots(n_rows, n_cols, figsize=(3 * n_cols, 3 * n_rows), squeeze=False)
        
        shown = 0
        for idx, rc in enumerate(individual[:n_rows * n_cols]):
            row, col = idx // n_cols, idx % n_cols
            ax = axes[row, col]
            
            # Get the individual cluster
            cluster = next((c for c in graph.clusters if c.id == rc['root_id']), None)
            if cluster:
                draw_cluster(ax, cluster)
                
                all_x = [s.pos2d[0] for s in cluster.sites]
                all_y = [s.pos2d[1] for s in cluster.sites]
                if all_x and all_y:
                    margin = 0.8
                    ax.set_xlim(min(all_x) - margin, max(all_x) + margin)
                    ax.set_ylim(min(all_y) - margin, max(all_y) + margin)
            ...
            ax.set_aspect('equal')
            ax.axis('off')
            ax.set_title(f"{rc['kinds'][0]} (ID={rc['root_id']})", fontsize=8)
            shown += 1
        
        # Hide unused subplots
        for idx in range(shown, n_rows * n_cols):
            row, col = idx // n_cols, idx % n_cols
            axes[row, col].axis('off')
        
        remaining = n_individual - shown
        title = f"Individual Aromatic Clusters ({n_individual} total)"
        if remaining > 0:
            title += f" - showing {shown}/{n_individual}"
        
        plt.suptitle(title, fontsize=12, fontweight='bold')
        plt.tight_layout()
        
        path = os.path.join(output_dir, "individual_clusters.png")
        plt.savefig(path, dpi=150, bbox_inches='tight', facecolor='white')
        plt.close(fig)
    
    print(f"[Visualization] Saved: {len(connected)} connected, {len(individual)} individual clusters")
